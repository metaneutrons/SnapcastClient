...
public class ResilientTcpConnection : IConnection, IDisposable
{
    ...
    public ResilientTcpConnection(string host, int port, SnapcastClientOptions? options = null, ILogger<ResilientTcpConnection>? logger = null)
    {
        ...
        {
            _healthCheckTimer = new Timer(PerformHealthCheck, null, TimeSpan.FromMilliseconds(_options.HealthCheckIntervalMs), 
    ...
    {
        if (_disposed) throw new ObjectDisposedException(nameof(ResilientTcpConnection));
    ...
    private async Task ConnectAsync()
    {
        ...
        while (attempt < _options.MaxRetryAttempts && !_disposed)
        {
            try
            {
                ...
                _logger?.LogInformation("Attempting to connect to {Host}:{Port} (attempt {Attempt}/{MaxAttempts})", 
                    _host, _port, attempt + 1, _options.MaxRetryAttempts);
